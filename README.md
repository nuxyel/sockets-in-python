...
Ok, entendido. Adicionei a seção para o `ssh_client.py` no `README.md`.

Aqui está a versão atualizada e completa do arquivo.

-----

# Sockets in Python

This repository contains a collection of Python scripts that utilize the `socket` and `paramiko` libraries for various networking tasks. These scripts were developed as a practical exercise while studying network programming and security concepts, inspired by courses from Solyd. The collection includes enhanced versions of standard examples, modified scripts for specific use cases, and original creations, all designed for educational and penetration testing study purposes.
...
An interactive SSH client that allows you to execute commands on a remote server. This script uses the `paramiko` library to handle the SSH connection.

**Features:**

  * **Interactive Shell**: Provides a prompt to execute multiple commands in a single session.
  * **Error Handling**: Displays both standard output and standard error from remote commands.
  * **Simplified Connection**: Connects using hardcoded credentials for quick tests.

**Dependencies:**
This script requires the `paramiko` library. You can install it using pip:
...
1.  Edit the `host`, `user`, and `passwd` variables inside the script with your target's credentials.
2.  Run the script from your terminal:
    ```bash
    python3 ssh_client.py
    ```
3.  You will see a `Comando:` prompt to start entering commands.
